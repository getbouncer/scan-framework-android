...
interface AnalyzerFactory<Output : Analyzer<*, *, *>> {
    suspend fun newInstance(forImmediateUse: Boolean): Output?
}
...
class AnalyzerPoolFactory<DataFrame, State, Output> @JvmOverloads constructor(
    ...
    suspend fun buildAnalyzerPool(forImmediateUse: Boolean) = AnalyzerPool(
        desiredAnalyzerCount = desiredAnalyzerCount,
        analyzers = (0 until desiredAnalyzerCount).mapNotNull { analyzerFactory.newInstance(forImmediateUse) }
    )
}
